...
export function WidgetLoader({ loaderStyle = 'skeleton', ambientBottomMargin = 0, showAmbientIcon = true }: WidgetLoaderProps) {
    ...
    const transparentRootStyle = `html, body, #__next, #root, .vion-widget-runtime-root { background: transparent !important; background-color: transparent !important; background-image: none !important; box-shadow: none !important; } body { --background: transparent !important; }`
    ...
    if (loaderStyle === 'spinner') {
        return (
            <div className="fixed inset-0 w-full overflow-hidden font-sans flex items-center justify-center z-50 pointer-events-none">
                <style dangerouslySetInnerHTML={{ __html: transparentRootStyle }} />
