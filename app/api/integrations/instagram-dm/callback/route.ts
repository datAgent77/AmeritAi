...
export async function GET(req: Request) {
    ...
    if (!code || !state) {
        return buildPopupResponse(origin, {
            type: "vion-instagram-dm-oauth",
            ok: false,
            error: providerError || "Meta doğrulaması tamamlanamadı.",
        })
    }

    const stateData = await consumeOAuthState(state, "integration-meta-instagram-dm")
    ...
    try {
        ...
        const firstPage = discovery.pages.find((page) => page.instagramAccount) || null
        ...
        let nextConfig = await mergeOmniChannelConfig(
            ...
            buildInstagramDMMergePayload({
                ...
                instagramAccountId: firstPage?.instagramAccount?.id || null,
                instagramUsername: firstPage?.instagramAccount?.username || null,
                pageAccessToken,
        ...
        if (firstPage) {
            try {
                ...
                const refreshed = await runInstagramDMPreflight(accessToken, chatbotId, adminDb)
                nextConfig = await mergeOmniChannelConfig(
                    ...
                    buildInstagramDMMergePayload({
                        ...
                        preflightResult: {
                            ...
                            webhookActive: true,
                            overallOk:
                                refreshed.result.hasFacebookPage === true &&
                                refreshed.result.instagramLinkedToPage === true &&
                                refreshed.result.instagramIsProfessional !== false &&
                                refreshed.result.messageAccessEnabled !== false &&
                                refreshed.result.tokenPresent === true,
                            failureReason:
                                refreshed.result.failureReason === "Mesaj akışı şu anda aktif görünmüyor." ? null : refreshed.result.failureReason,
                        },
        ...
        await chatbotRef.set(
            {
                integrations: {
                    instagram: {
                        ...
                        pageId: firstPage?.id || null,
                        accountId: firstPage?.instagramAccount?.id || null,
                        accessToken: pageAccessToken,
                        verifyToken: nextConfig?.instagram?.verifyToken || appConfig.verifyToken,
        ...
        return buildPopupResponse(origin, {
            type: "vion-instagram-dm-oauth",
            ok: false,
            error: error instanceof Error ? error.message : "Instagram DM bağlantısı tamamlanamadı.",
        })
    }
}
