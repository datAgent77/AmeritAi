...
describe("POST /api/omni/channels/whatsapp/test-message", () => {
    beforeEach(() => {
        vi.clearAllMocks()
    ...
    test("records disabled channel as blocked instead of error", async () => {
        const response = await POST(
            new Request("https://preview.example.com/api/omni/channels/whatsapp/test-message", {
                method: "POST",
        ...
        expect(await response.json()).toEqual({ error: "WhatsApp channel is disabled" })
        ...
        expect(sendOmniWhatsAppText).not.toHaveBeenCalled()
    })
})
