...
export function AppearanceTab({
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Auto Open Behavior */}
            <AccordionItem value="auto-open" className="border-none">
                <div className="space-y-3">
                    <AccordionTrigger className="hover:no-underline border rounded-lg px-4 bg-card shadow-sm [&[data-state=open]]:rounded-b-none">
                        <span className="text-sm font-medium">{language === 'tr' ? 'Otomatik Açılış' : 'Auto Open'}</span>
                    </AccordionTrigger>
                    <AccordionContent className="border border-t-0 rounded-b-lg px-4 pt-4 pb-6 bg-card shadow-sm !mt-0">
                        <div className="space-y-4">
                            <p className="text-xs text-muted-foreground">
                                {language === 'tr'
                                    ? 'Klasik launcher modunda ziyaretçi davranışına göre widgetı otomatik açar. Baloncuk mesajları için Proaktif Etkileşim modülündeki tetikleyicileri kullanın.'
                                    : 'Automatically opens the widget in classic launcher mode based on visitor behavior. Use Proactive Engagement triggers for bubble messages.'}
                            </p>

                            {isAlwaysOpenMode && (
                                <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                                    {language === 'tr'
                                        ? 'Mevcut görünüm her zaman açık olduğu için bu tetikleyiciler yalnızca klasik launcher modunda etkili olur.'
                                        : 'The current display mode is already open; these triggers only affect classic launcher mode.'}
                                </div>
                            )}
                            ...
                            <div className="grid gap-4">
                                <div className="flex items-start justify-between gap-4 rounded-lg border p-4">
                                    <div className="space-y-1">
                                        <Label className="text-sm font-medium">
                                            {language === 'tr' ? 'Çıkış niyetinde aç' : 'Open on exit intent'}
                                        </Label>
                                        <p className="text-xs text-muted-foreground">
                                            {language === 'tr'
                                                ? 'Masaüstünde kullanıcı fareyi tarayıcı üst alanına taşıdığında widgetı açar.'
                                                : 'Opens the widget on desktop when the visitor moves the pointer toward the browser chrome.'}
                                        </p>
                                    </div>
                                    <Switch
                                        checked={settings.openOnExitIntent}
                                        onCheckedChange={(checked) => setSettings(prev => ({ ...prev, openOnExitIntent: checked }))}
                                        disabled={isAlwaysOpenMode}
                                    />
                                </div>
                                ...
                                <div className="rounded-lg border p-4 space-y-3">
                                    <div className="flex items-start justify-between gap-4">
                                        <div className="space-y-1">
                                            <Label htmlFor="open-on-scroll" className="text-sm font-medium">
                                                {language === 'tr' ? 'Sayfa derinliğinde aç' : 'Open at scroll depth'}
                                            </Label>
                                            <p className="text-xs text-muted-foreground">
                                                {language === 'tr'
                                                    ? 'Ziyaretçi sayfanın belirlenen yüzdesine indiğinde widget açılır. 0 kapalı demektir.'
                                                    : 'Opens when the visitor reaches the selected page depth. 0 disables it.'}
                                            </p>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <Input
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    setSettings(prev => ({ ...prev, openOnScroll: nextValue }))
                                                }}
                                                disabled={isAlwaysOpenMode}
                                            />
                                ...
                                <div className="rounded-lg border p-4 space-y-3">
                                    <div className="flex items-start justify-between gap-4">
                                        <div className="space-y-1">
                                            <Label htmlFor="auto-open-delay" className="text-sm font-medium">
                                                {language === 'tr' ? 'Süre sonunda aç' : 'Open after delay'}
                                            </Label>
                                            <p className="text-xs text-muted-foreground">
                                                {language === 'tr'
                                                    ? 'Sayfa yüklendikten kaç saniye sonra widgetın açılacağını belirler. 0 kapalı demektir.'
                                                    : 'Sets how many seconds after page load the widget opens. 0 disables it.'}
                                            </p>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <Input
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    setSettings(prev => ({ ...prev, autoOpenDelay: nextValue }))
                                                }}
                                                disabled={isAlwaysOpenMode}
                                            />
